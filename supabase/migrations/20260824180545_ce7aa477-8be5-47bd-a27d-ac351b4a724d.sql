CREATE OR REPLACE FUNCTION public.registrar_pedido(_telefone text DEFAULT NULL::text, _valor_pedido numeric DEFAULT 0, _bling_id text DEFAULT NULL::text, _data_pedido timestamp with time zone DEFAULT now(), _id_pedido text DEFAULT NULL::text, _nome_cliente text DEFAULT NULL::text)
 RETURNS SETOF customers
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  _customer_id uuid;
  _normalized_phone text;
  _effective_phone text;
  _has_pipeline_entry boolean;
BEGIN
  _normalized_phone := public.normalize_brazil_phone_e164(_telefone);

  IF _bling_id IS NOT NULL THEN
    SELECT id INTO _customer_id FROM public.customers WHERE bling_id = _bling_id;
  END IF;

  IF _customer_id IS NULL THEN
    IF _bling_id IS NULL THEN
      RAISE EXCEPTION 'bling_id (ID Contato) é obrigatório para identificar o cliente';
    END IF;

    INSERT INTO public.customers (
      telefone, bling_id, nome, data_primeiro_contato, data_conversao
    ) VALUES (
      COALESCE(_normalized_phone, 'bling_' || _bling_id),
      _bling_id, _nome_cliente, _data_pedido, _data_pedido
    )
    RETURNING id INTO _customer_id;
  END IF;

  IF _id_pedido IS NOT NULL THEN
    INSERT INTO public.orders (customer_id, id_pedido, valor, data_pedido)
    VALUES (_customer_id, _id_pedido, _valor_pedido, _data_pedido)
    ON CONFLICT (id_pedido) DO NOTHING;
  ELSE
    INSERT INTO public.orders (customer_id, valor, data_pedido)
    VALUES (_customer_id, _valor_pedido, _data_pedido);
  END IF;

  UPDATE public.customers
  SET
    telefone = CASE
      WHEN _normalized_phone ~ '^55\d{10,11}$' THEN _normalized_phone
      ELSE telefone
    END,
    total_pedidos = (SELECT count(*) FROM public.orders WHERE customer_id = _customer_id),
    valor_total_comprado = (SELECT COALESCE(sum(valor), 0) FROM public.orders WHERE customer_id = _customer_id),
    data_ultimo_pedido = (SELECT max(data_pedido) FROM public.orders WHERE customer_id = _customer_id),
    data_conversao = COALESCE(data_conversao, _data_pedido),
    nome = COALESCE(nome, _nome_cliente)
  WHERE id = _customer_id;

  -- Fallback: se o telefone não veio no payload, usa o telefone já cadastrado no cliente
  IF _normalized_phone ~ '^55\d{10,11}$' THEN
    _effective_phone := _normalized_phone;
  ELSE
    SELECT telefone INTO _effective_phone FROM public.customers WHERE id = _customer_id;
    IF _effective_phone IS NOT NULL AND _effective_phone !~ '^55\d{10,11}$' THEN
      _effective_phone := NULL;
    END IF;
  END IF;

  IF _effective_phone IS NOT NULL THEN
    -- Venda concluída: o card do funil é marcado automaticamente como CLIENTE.
    -- O ciclo só reinicia quando a pessoa mandar uma nova mensagem (trigger de reset).
    UPDATE public.contatos
    SET status_funil = 'cliente',
        ultima_interacao = COALESCE(ultima_interacao, _data_pedido)
    WHERE telefone = _effective_phone
      AND status_funil IS DISTINCT FROM 'cliente';

    UPDATE public.leads_pipeline
    SET popup_exibido = true,
        popup_ciclo_data = CURRENT_DATE,
        convertido = true,
        data_interacao = now(),
        data_ultima_interacao = now()
    WHERE telefone = _effective_phone
      AND convertido = false;

    SELECT EXISTS (
      SELECT 1
      FROM public.leads_pipeline
      WHERE telefone = _effective_phone
        AND popup_exibido = true
        AND popup_ciclo_data = CURRENT_DATE
    ) INTO _has_pipeline_entry;

    IF NOT _has_pipeline_entry THEN
      INSERT INTO public.leads_pipeline (
        telefone, nome, etapa_pipeline, status, convertido, salvo_manualmente,
        origem_tentativa, popup_exibido, popup_ciclo_data,
        data_interacao, data_entrada, data_ultima_interacao
      ) VALUES (
        _effective_phone, _nome_cliente, 'proposta_sem_resposta', 'perdido',
        true, false, 'pipeline', true, CURRENT_DATE, now(), now(), now()
      );
    END IF;
  END IF;

  RETURN QUERY SELECT * FROM public.customers WHERE id = _customer_id;
END;
$function$;