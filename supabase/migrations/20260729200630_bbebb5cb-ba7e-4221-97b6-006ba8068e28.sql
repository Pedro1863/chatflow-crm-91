...
BEGIN
  ...
  IF _normalized_phone ~ '^55\d{10,11}$' THEN
    -- Venda concluída: o ciclo do funil reinicia (card volta para o início,
    -- aparecendo na aba de Recompras). Não usamos mais o status travado 'cliente'.
    UPDATE public.contatos
    SET status_funil = 'novo_lead',
        ultima_interacao = COALESCE(ultima_interacao, _data_pedido)
    WHERE telefone = _normalized_phone
      AND status_funil IS DISTINCT FROM 'novo_lead';
