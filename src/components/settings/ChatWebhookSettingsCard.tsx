...
const ChatWebhookSettingsCard = () => {
  const { data: savedUrl, isLoading } = useSystemSetting("n8n_chat_webhook_url");
  ...
  const handleSave = () => {
    ...
    updateSetting.mutate(
      { key: "n8n_chat_webhook_url", value: url },
      {
        onSuccess: () => toast.success("Webhook de chat salvo com sucesso!"),
    ...
    try {
      const res = await fetch(url.replace(/\/$/, ""), {
        ...
        body: JSON.stringify({
          ...
          mensagem: "Teste de conexão do dashboard",
        }),
      });
    ...
    <Card className="border-chart-2/30">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-base">Webhook URL (n8n) — Mensagens do Chat</CardTitle>
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        <p className="text-sm text-muted-foreground">
          URL do webhook n8n usada para enviar mensagens digitadas nas conversas abertas.
          Separada do webhook de templates.
        </p>
        ...
        {!url && savedUrl === "" && (
          <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-3">
            <p className="text-xs text-destructive flex items-center gap-1.5">
              ...
              Webhook de chat não configurado. O envio de mensagens pelo painel não funcionará.
            </p>
          </div>
        )}
...
export default ChatWebhookSettingsCard;
